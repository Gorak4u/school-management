...
export const PaymentHistoryModal: React.FC<PaymentHistoryModalProps> = ({ isOpen, student, fees, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
      <div className="bg-white rounded-2xl max-w-3xl w-full p-6 shadow-2xl animate-scaleIn max-h-[90vh] flex flex-col">
        <div className="flex justify-between items-center mb-4 border-b border-slate-100 pb-4">
          <div className="flex items-center gap-3">
            <History className="text-indigo-600" size={24} />
            <div>
              <h2 className="text-xl font-bold text-slate-800">Payment History</h2>
              <p className="text-sm text-slate-500">{student.name} ({student.standard} - {student.section})</p>
            </div>
          </div>
            ...
            <table className="w-full text-sm text-left">
              <thead className="bg-slate-50 sticky top-0">
                <tr>
                  ...
                  <th className="px-4 py-3 font-semibold text-slate-600">Receipt Ref.</th>
                  ...
                  <th className="px-4 py-3 font-semibold text-slate-600">Receiver</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {paymentHistory.map((payment, index) => (
                  <tr key={index} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-slate-600 font-mono text-xs">{payment.refNo || '-'}</td>
                    ...
                    <td className="px-4 py-3 text-slate-600 text-xs">{payment.receiverName || 'System'}</td>

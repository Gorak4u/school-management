...
export const Teachers: React.FC<TeachersProps> = ({ teachers, onAddTeacher, onUpdateTeacher, onDeleteTeacher, settings }) => {
  ...
  // Derived available standards based on selected medium in the form
  const availableStandards = useMemo(() => {
    if (settings.mediumSpecificStandards && settings.mediumSpecificStandards[assignedMed]) {
        return settings.mediumSpecificStandards[assignedMed];
    }
    return settings.standards;
  }, [assignedMed, settings]);

  // Reset assigned standard if it's no longer valid for the new medium
  useEffect(() => {
      if (!availableStandards.includes(assignedStd)) {
          setAssignedStd(availableStandards[0] || '');
      }
  }, [availableStandards, assignedStd]);
  ...
  const handleEdit = (teacher: Teacher) => {
    ...
    const med = teacher.assignedMedium || settings.mediums[0];
  ...
  const handleAddNew = () => {
    ...
    // Standard will auto-update via effect
    setAssignedSec('A');
    setFormData({
      name: '',
      role: 'Teacher',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Unique Class Teacher Check
    if (formData.isClassTeacher) {
      const duplicate = teachers.find(t => 
        t.isClassTeacher &&
        t.assignedClass === finalAssignedClass &&
        t.assignedMedium === finalAssignedMedium &&
        t.id !== (editingTeacher?.id || '')
      );

      if (duplicate) {
        alert(`Error: Class teacher already assigned for ${finalAssignedClass} (${finalAssignedMedium}) - ${duplicate.name}`);
        return;
      }
    }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {filteredTeachers.map(teacher => (
          <div key={teacher.id} className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 text-center flex flex-col group transition-all hover:shadow-md hover:border-indigo-100">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <div className="bg-white rounded-xl max-w-lg w-full p-6 shadow-xl">
                <div className="flex justify-between items-center mb-4"><h2 className="text-xl font-bold text-slate-800">{editingTeacher ? 'Edit Staff Details' : 'Add New Staff Member'}</h2><button onClick={() => setIsModalOpen(false)}><X/></button></div>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="flex items-center gap-4"><div className="w-16 h-16 rounded-full bg-slate-100 overflow-hidden relative group border border-slate-200">{formData.photo ? <img src={formData.photo} className="w-full h-full object-cover"/> : <User className="w-full h-full p-3 text-slate-300"/>}<label className="absolute inset-0 bg-black/40 flex items-center justify-center text-white opacity-0 group-hover:opacity-100 cursor-pointer transition-opacity"><Upload size={20}/><input type="file" accept="image/*" className="hidden" onChange={handlePhotoUpload}/></label></div><div><label className="text-sm font-medium text-slate-700">Profile Photo</label><div className="flex items-center gap-2 mt-1"><label className="text-xs font-semibold bg-slate-100 hover:bg-slate-200 px-2 py-1 rounded cursor-pointer">Change...<input type="file" accept="image/*" className="hidden" onChange={handlePhotoUpload}/></label>{formData.photo && <button type="button" onClick={removePhoto} className="text-xs text-red-600">Remove</button>}</div></div></div>
                    ...
                    {formData.role === 'Teacher' && (
                        <div className="p-4 bg-indigo-50 rounded-lg border border-indigo-100 space-y-4">
                            <div><label className="text-xs font-medium">Subject Specialization</label><input value={formData.subjectSpecialization} onChange={e => setFormData({...formData, subjectSpecialization: e.target.value})} className="w-full p-2 border rounded text-sm bg-white"/></div>
                            <div className="flex flex-col gap-3">
                                <div className="flex items-center gap-2"><input type="checkbox" id="isClassTeacher" checked={formData.isClassTeacher} onChange={e => setFormData({...formData, isClassTeacher: e.target.checked})} className="rounded text-indigo-600"/><label htmlFor="isClassTeacher" className="text-sm font-medium text-indigo-900">Is Class Teacher?</label></div>
                                {formData.isClassTeacher && (
                                    <div className="grid grid-cols-3 gap-2 animate-fadeIn">
                                        <div>
                                            <label className="text-xs text-indigo-700 font-medium">Medium</label>
                                            <select value={assignedMed} onChange={e => setAssignedMed(e.target.value)} className="w-full p-2 border border-indigo-200 rounded text-sm bg-white outline-none">
                                                {settings.mediums.map(m => <option key={m} value={m}>{m}</option>)}
                                            </select>
                                        </div>
                                        <div>
                                            <label className="text-xs text-indigo-700 font-medium">Class</label>
                                            <select value={assignedStd} onChange={e => setAssignedStd(e.target.value)} className="w-full p-2 border border-indigo-200 rounded text-sm bg-white outline-none">
                                                {availableStandards.map(s => <option key={s} value={s}>{s}</option>)}
                                            </select>
                                        </div>
                                        <div>
                                            <label className="text-xs text-indigo-700 font-medium">Section</label>
                                            <select value={assignedSec} onChange={e => setAssignedSec(e.target.value)} className="w-full p-2 border border-indigo-200 rounded text-sm bg-white outline-none">

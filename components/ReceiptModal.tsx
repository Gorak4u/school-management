...
export const ReceiptModal: React.FC<ReceiptModalProps> = ({ isOpen, student, fees, settings, onClose }) => {
  ...
  // Find the latest payment reference number across all relevant fee records
  const latestRefNo = useMemo(() => {
    let latestDate = 0;
    let ref = '';
    
    receiptFees.forEach(f => {
        if (f.payments) {
    ...
    return ref || `TMP-${Math.floor(Math.random() * 100000)}`; // Fallback if no payment made yet
  }, [receiptFees]);
          ...
          <div className="p-8 print:p-8" id="printable-receipt">
            ...
            <div className="flex justify-between mb-8">
              ...
              <div className="text-right">
                ...
                <p className="text-sm text-slate-600 font-mono font-bold">Ref: {latestRefNo}</p>
              </div>
            </div>

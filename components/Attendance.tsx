...
export const Attendance: React.FC<AttendanceProps> = ({ students, onSave }) => {
  ...
  // Derive lists from student data to allow dynamic selection
  const availableMediums = Array.from(new Set(students.map(s => s.medium))).sort();
  
  // Refined standard list based on selected medium from students data 
  // (In a real scenario, passing 'settings' prop is cleaner, but this works based on data)
  const availableStandards = Array.from(new Set(students.filter(s => s.medium === medium).map(s => s.standard))).sort((a: string, b: string) => {

...
import { Student, FeeRecord, Homework, SMSLog, Exam, MarkRecord, SchoolSettings, Teacher, BusRoute, FeeStructure, Standard, WhatsAppGroup, ExpenseRecord, ExpenseCategory, SalaryRecord, SalaryAdvance, Timetable, SchoolEvent, User } from './types';
import { loadData, saveData, performHardReset, getAllData } from './utils/db'; 
...
function App() {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      const skipMocks = localStorage.getItem('SVS_SKIP_MOCKS') === 'true';
      const [ loadedAuth, loadedUser, loadedStudents, loadedFees, loadedHomework, loadedRoutes, loadedTeachers, loadedFeeStructure, loadedSms, loadedExams, loadedMarks, loadedSettings, loadedExpenses, loadedExpCats, loadedSalaries, loadedAdvances, loadedTimetables, loadedEvents, loadedAppState, loadedUsers ] = await Promise.all([
        ...
        loadData('students', skipMocks ? [] : MOCK_STUDENTS),
        loadData('fees', skipMocks ? [] : MOCK_FEES),
        loadData('homework', []),
        loadData('routes', skipMocks ? [] : INITIAL_BUS_ROUTES),
        loadData('teachers', skipMocks ? [] : INITIAL_TEACHERS),
  ...
  const performGithubBackup = async () => {
      if (!githubPat || !githubRepo || !navigator.onLine || isResetting.current) return;
      ...
      } catch (e) { setGithubSyncStatus('error'); }
  };
  ...
  const handleRepopulate = async () => { if (isResetting.current) return; isResetting.current = true; try { await performHardReset(); localStorage.removeItem('SVS_SKIP_MOCKS'); alert("Data reset successfully!"); window.location.reload(); } catch (err) { isResetting.current = false; } };
  const handleFormat = async () => { if (isResetting.current) return; isResetting.current = true; try { await performHardReset(); localStorage.setItem('SVS_SKIP_MOCKS', 'true'); alert("Data formatted successfully!"); window.location.reload(); } catch (err) { isResetting.current = false; } };
  ...
  const handleUpdateFee = (feeId: string, paymentAmount: number, paymentMode: string) => { setFees(fees.map(f => { if (f.id === feeId) { const newPayment = { date: new Date().toISOString().split('T')[0], amount: paymentAmount, mode: paymentMode }; const updatedPayments = [...(f.payments || []), newPayment]; const newPaidAmount = updatedPayments.reduce((sum, p) => sum + p.amount, 0); const newStatus = newPaidAmount >= f.amount ? 'Paid' : 'Partial'; return { ...f, payments: updatedPayments, paidAmount: newPaidAmount, status: newStatus, datePaid: newPayment.date }; } return f; })); };
  ...
  const handleChangePassword = async (oldPass: string, newPass: string): Promise<boolean> => {
    ...
    setCurrentUser(updatedUser); // Update session
    return true;
  };
  ...
  // Called from Login component when using Recovery Key
  ...
  if (loading) return <div className="flex items-center justify-center h-screen bg-slate-100 text-slate-500 font-semibold text-lg"><div className="animate-pulse">Loading SVS Database...</div></div>;
  ...
  return (
    <>
      <Layout activeTab={activeTab} setActiveTab={setActiveTab} settings={settings} onLogout={handleLogout} saveStatus={saveStatus} lastGithubPushTime={lastGithubPushTime} githubSyncStatus={githubSyncStatus} currentUser={currentUser} onChangePassword={handleChangePassword}>
        {activeTab === 'dashboard' && <Dashboard students={students} fees={fees} expenses={expenses} salaryRecords={salaryRecords} settings={settings} onStudentSelect={setQuickViewStudent} onQuickLinkClick={handleQuickLink} busRoutes={busRoutes} />}
      ...
      <StudentQuickViewModal isOpen={!!quickViewStudent} student={quickViewStudent} fees={fees.filter(f => f.studentId === quickViewStudent?.id)} busRoutes={busRoutes} settings={settings} onClose={() => setQuickViewStudent(null)} onUpdateFee={handleUpdateFee} onEditStudent={(s) => { setActiveTab('students'); setStudentToEditFromDashboard(s); setQuickViewStudent(null); }} onShowReceipt={setReceiptModalStudent} onShowCertificate={setCertificateModalStudent} onShowHistory={setHistoryModalStudent} />
      <ReceiptModal isOpen={!!receiptModalStudent} student={receiptModalStudent} fees={fees} settings={settings} onClose={() => setReceiptModalStudent(null)} />

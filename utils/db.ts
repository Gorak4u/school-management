...
export const restoreAllData = async (data: Record<string, any>) => {
    // NOTE: We do NOT perform a Hard Reset here anymore.
    // This allows partial restores (e.g. from Excel) without wiping settings/images.
    ...
    await Promise.all(
        existingStoreNames.map(async (storeName) => {
            if (Object.prototype.hasOwnProperty.call(data, storeName)) {
                const store = tx.objectStore(storeName);
                
                // SPECIAL HANDLING FOR SETTINGS
                // If we are restoring settings, we want to preserve images (logo, etc)
                // if the imported data doesn't have them (e.g. Excel import).
                if (storeName === 'settings') {
                    const currentSettings = await store.get('data') as any || {};
                    const newSettings = data[storeName];

                    // Merge images from current if missing in new
                    if (!newSettings.schoolLogo && currentSettings.schoolLogo) {
                        newSettings.schoolLogo = currentSettings.schoolLogo;
                    }
                    if (!newSettings.schoolBanner && currentSettings.schoolBanner) {
                        newSettings.schoolBanner = currentSettings.schoolBanner;
                    }
                    if (!newSettings.principalSignature && currentSettings.principalSignature) {
                        newSettings.principalSignature = currentSettings.principalSignature;
                    }
                    
                    // Also preserve WhatsApp groups if not in import
                    if ((!newSettings.whatsAppGroups || newSettings.whatsAppGroups.length === 0) && currentSettings.whatsAppGroups?.length > 0) {
                        newSettings.whatsAppGroups = currentSettings.whatsAppGroups;
                    }

                    await store.put(newSettings, 'data');
                } else {
                    // For other stores (students, fees, etc.), if data is provided,
                    // we replace the content of that store.
                    await store.clear();
                    await store.put(data[storeName], 'data');
                }
            }
        })
    );
